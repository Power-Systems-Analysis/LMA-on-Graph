% Тоже, что и funEckij_sym, только считает для случая i = j

function[Eckii] = funEckii_sym(linmod,n_mat,n_node,c)
...
try
    %processing status
    f = waitbar(0,'Starting...','Name','Eсkii_sym calculation...',...
    ...
    denom_conj = -1 ./ (4*real(linmod.Dseq));
    denom = -1 ./ (4*linmod.Dseq);
    na = 0;
    nn = 0;
    for a = n_mat
        ...
        for i = id_a
            ...
            conjtrRi = Ri';
            trRi = Ri.';
            Rjdenom_conj = Ri * denom_conj (i,a);
            Rjdenom = Ri * denom (i,a);
            for kth = n_node
                nn = nn + 1;
                Qk = conjtr_cv(:,kth,a) * c(kth,:,a);
                Pvkij = (conjtrRi * Qk * Rjdenom_conj + trRi * Qk * Rjdenom);
                Eckii(nn,i,na) = trace(Pvkij);
            end
            nn = 0;
